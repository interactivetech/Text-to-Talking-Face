...
def main():
    ...
    while cap.isOpened():
        ...
        if len(faces) == 1:
            for face in faces:
                # d = predictor(gray, face)
                # Crop frame for face region only
                black = np.zeros((256,256,3),dtype=np.uint8)
                center = 256//2#128
                crop_img = frame_resize[face.top():face.bottom(),face.left():face.right()]
                ...
                top=center - int(ch)
                if h%2==1:
                    bottom = center+int(ch)+1
                elif h%2==0:
                    bottom = center+int(ch)
                
                left = center-int(cw)
                if w%2==1:
                    right = center+int(cw)+1
                elif w%2==0:
                    right = center+int(cw)

                black[top:bottom,left:right] = crop_img
            ...
            cv2.imwrite("crop/{:09d}.png".format(count), black)
            # tmpimage = frame.copy()
