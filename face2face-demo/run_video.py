...
def resize(image):
    ...
    # print("CROP: {},{}".format(height,width))
    if height != width:
...
def main():
    ...
    # Second Video
    cap2 = cv2.VideoCapture("/home/face2face-demo/initial_data/Shannon_2_vid_crop_audio.mp4")
    print(cap)
    print(cap2)
    ...
    for _ in tqdm(range(total)):
        ...
        frame_resize2 = cv2.resize(frame2, None, fx=1 / DOWNSAMPLE_RATIO, fy=1 / DOWNSAMPLE_RATIO)

        gray = cv2.cvtColor(frame_resize, cv2.COLOR_BGR2GRAY)
        faces = detector(gray, 1)
        black_image = np.zeros(frame.shape, np.uint8)
        black_image=frame_resize
        # for face in faces:
        ...
        # generate prediction
        combined_image = np.concatenate([resize(black_image), resize(frame_resize2)], axis=1)
        ...
        image_all = np.concatenate([resize(frame_resize2), resize(black_image), image_bgr], axis=1)
        ...
        cv2.imwrite('/home/face2face-demo/pix2pix-tensorflow/res/image%09d.jpg'%counter,image_all)
        cv2.imwrite('/home/face2face-demo/pix2pix-tensorflow/res/face/gen/image%09d.jpg'%counter,image_bgr)
        cv2.imwrite('/home/face2face-demo/pix2pix-tensorflow/res/face/face/image%09d.jpg'%counter,resize(black_image))
        cv2.imwrite('/home/face2face-demo/pix2pix-tensorflow/res/face/input/image%09d.jpg'%counter,resize(frame_resize))
        if len(faces)>0:
            cv2.imwrite('/home/face2face-demo/pix2pix-tensorflow/res/face/mix/image%09d.jpg'%counter,image_bgr)
        else:
            cv2.imwrite('/home/face2face-demo/pix2pix-tensorflow/res/face/mix/image%09d.jpg'%counter,resize(frame_resize))
